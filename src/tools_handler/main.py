...
nc, js, tool_use_sub, get_weather_sub = None, None, None, None
    ...
    while retry_count < max_retries and running:
        try:
            ...
            try:
                ...
                await js.stream_info("tools_handle")
...
async def process_tools(msg):
    """
    Handles an incoming results from the tools, tracing the process of tools
    Mark as completions and handle back to the LLM.
    """
    pass


async def handle_tools(msg):
    """
    Handles an incoming messages, assign tools, handle calling tools.
    """
...
async def start_subscription():
    """
    Subscribe to 'tool_use' and 'tool_result' subject using JetStream durable consumer.
    """
    global js, tool_use_sub, get_weather_sub, running
    if not js:
    ...
    try:
        subscribe_opts = ConsumerConfig(
            durable_name="tools_handler",
            ack_policy=AckPolicy.EXPLICIT,
            ack_wait=60 * 5,
            max_deliver=5,
        )

        # Subscribe to the 'tool_use' subject with the durable consumer
        tool_use_sub = await js.subscribe(
            subject="tool_use",
            cb=process_tools,
            manual_ack=True,
            stream="tools_handle",
        ...
        info = "Subscribed to 'tool_use' subject with durable consumer 'tools_handler'"
        logger.info(info)

        # Subcribe to the 'get_weather' subject with the durable consumer
        get_weather_sub = await js.subscribe(
            subject="tool_result",
            cb=handle_tools,
            manual_ack=True,
            stream="tools_handle",
        ...
        info = "Subscribed to 'tool_result' subject with durable consumer 'tools_handler'"  # noqa: E501
        logger.info(info)
...
async def shutdown():
    ...
    logger.info("Shutting down tools handle server...")
...
async def main():
    ...
    if await connect_to_nats():
        ...
        logger.error("Failed to connect to NATS. Server cannot start.")
